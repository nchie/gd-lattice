...
func _on_request_refresh_plugin(p_name):
	assert(not p_name.length() == 0)
	
	var p_cfg_path = ADDONS_PATH.plus_file(p_name).plus_file("plugin.cfg")

	var disabled = not get_editor_interface().is_plugin_enabled(p_cfg_path)
	if disabled:
		refresher.show_warning(p_cfg_path)
	else:
		refresh_plugin(p_cfg_path)
...
func refresh_plugin(p_cfg_path):
	print("Refreshing plugin: ", p_cfg_path)

	var enabled = get_editor_interface().is_plugin_enabled(p_cfg_path)
	if enabled: # can only disable an active plugin
		get_editor_interface().set_plugin_enabled(p_cfg_path, false)

	get_editor_interface().set_plugin_enabled(p_cfg_path, true)

	plugin_config.set_value(SETTINGS, SETTING_RECENT, p_cfg_path)
	_save_settings()
